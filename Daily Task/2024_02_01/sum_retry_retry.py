...
#     while cnt != N**2:
#         if 0 <= i+di[k] <= N-1 and 0 <= j+dj[k] <= N-1 and arr[i+di[k]][j+dj[k]] == 0:
#             i += di[k]
#             j += dj[k]
#             cnt += 1
#             arr[i][j] = cnt
...
for test_case in range(1,T+1):
    ...
    arr_sum = [0]*202
    for i in range(100):
        arr_sum[101] += arr[i][i] # 대각선1 합 arr_sum [101]
        arr_sum[0] += arr[i][99-i] # 대각선2 합 arr_sum[0]
        for j in range(100):
            arr_sum[201-i] += arr[i][j] # 가로 합 arr_sum[102~201]
            arr_sum[i+1] += arr[j][i] # 세로 합 arr_sum[1~100]
    
    print(f'#{test_case} {max(arr_sum)}')